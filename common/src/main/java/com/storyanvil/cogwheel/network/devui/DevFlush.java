...
public record DevFlush(Identifier lc) implements StoryPacket<DevFlush> {
    public static final StoryCodec<DevFlush> CODEC = StoryCodecBldr.build(
            StoryCodecBldr.Prop(DevFlush::lc, StoryCodecs.RESOURCE_LOC),
            DevFlush::new
    );

    @Override
    public StoryCodec<DevFlush> getStoryCodec() {
        return CODEC;
    }
