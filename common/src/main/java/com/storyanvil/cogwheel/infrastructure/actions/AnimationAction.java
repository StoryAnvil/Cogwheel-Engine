...
public class AnimationAction extends StoryAction<StoryAnimator> {
    ...
    public void proceed(@NotNull StoryAnimator myself) {
        CogwheelHooks.sendPacketToEveryone(new AnimationBound(myself.getAnimatorID(), animation));
    }
    ...
    public boolean freeToGo(StoryAnimator myself) {
        ticks--;
        if (ticks <= 0) {
            CogwheelHooks.sendPacketToEveryone(new AnimationBound(myself.getAnimatorID(), "null"));
            return true;
        }
        return false;
    }
