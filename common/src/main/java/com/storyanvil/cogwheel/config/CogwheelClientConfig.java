...
public class CogwheelClientConfig {
    ...
    private static String getString(String name, String defauld) {
        JsonElement e = json.get(name);
        if (e == null || !e.isJsonPrimitive()) {
            json.addProperty(name, defauld);
            return defauld;
    ...
    public static synchronized void reload() {
        ...
        try (FileReader fr = new FileReader(config); Scanner sc = new Scanner(fr)) {
            ...
            try {
                ...
                throw new RuntimeException(config.toString() + " while creating config-client", ex);
            }
        } catch (IOException e) {
        ...
        ide = getBool("isRunningInIDE", false);
    ...
    public static boolean isShowingAllLogs() {
        return showAllLogs;
    }

    public static boolean isTraceLogLevelEnabled() {
        return enableTraceLogs;
    }
