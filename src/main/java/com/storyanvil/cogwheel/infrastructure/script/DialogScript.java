        ...
        switch (code.charAt(0)) {
            ...
            case '/' -> {
                CogwheelExecutor.scheduleTickEvent(levelTickEvent -> {
                    levelTickEvent.level.getServer().getCommands().performPrefixedCommand(
                            new CommandSourceStack(CommandSource.NULL, new Vec3(0, 0, 0),
                                    new Vec2(0, 0), (ServerLevel) levelTickEvent.level, 4, "COGWHEEL", Component.literal("COGWHEEL"), levelTickEvent.level.getServer(), null),
                            code.substring(1));
                });
            }
            case '@' -> {
                ...
                CogPropertyManager unchecked = super.get(code.substring(1, actPlace));
    ...
    private @NotNull String format(@NotNull String raw, @Nullable B<Integer> ticks) {
        ...
        for (int i = 0; i < raw.length(); i++) {
            ...
            if (c == '$' && i + 3 < raw.length() && raw.charAt(i + 1) == '{') {
                ...
                CogPropertyManager m = super.fastExecute(expr.toString());
