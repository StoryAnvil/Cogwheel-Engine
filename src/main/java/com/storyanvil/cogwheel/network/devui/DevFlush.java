...
public record DevFlush(ResourceLocation lc) implements StoryPacket {
    public static final StoryCodec<DevFlush> CODEC = StoryCodecBuilder.build(
            StoryCodecBuilder.Prop(DevFlush::lc, StoryCodecs.RESOURCE_LOC),
            DevFlush::new
    );
    ...
    public void onServerUnsafe(Supplier<NetworkEvent.Context> ctx) {
        ...
        session.flush(ctx.get().getSender());
    }
}
