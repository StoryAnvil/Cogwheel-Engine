...
public class DialogChoiceScreen extends Screen {
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        for (int i = 0; i < bound.getOptions().length; i++) {
            ...
            guiGraphics.fill(left, top, responseEnd, bottom, isHovered ? -936892376 : -939524096);
            guiGraphics.drawWordWrap(font, FormattedText.of(o), responseX, responseY[i], responseWidth, 16777215);
        }
    }
    ...
    public void resize(@NotNull Minecraft minecraft, int width, int height) {
        ...
        responseY = new int[bound.getOptions().length];
        responseH = new int[bound.getOptions().length];
        ...
        for (int i = 0; i < bound.getOptions().length; i++) {
            String o = bound.getOptions()[i];
            int lines = font.split(FormattedText.of(o), responseWidth).size();
            int h = lines * font.lineHeight;
            responseY[i] = baseY - h;
            responseH[i] = h;
