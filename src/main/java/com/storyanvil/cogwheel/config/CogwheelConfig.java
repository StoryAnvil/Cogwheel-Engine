...
public class CogwheelConfig {
    ...
    public static synchronized void reload() {
        File config = new File(Minecraft.getInstance().gameDirectory, "config/cog/config-main.json");
        ...
        HashSet<ResourceLocation> disabledLH = new HashSet<>();
        for (String lh : disabledLineHandlers) {
            ResourceLocation rl = ResourceLocation.parse(lh);
            disabledLH.add(rl);
        }
        for (Bi<ScriptLineHandler, Boolean> lh : CogwheelRegistries.getLineHandlers()) {
            lh.setB(!disabledLH.contains(lh.getA().getResourceLocation()));
        }
