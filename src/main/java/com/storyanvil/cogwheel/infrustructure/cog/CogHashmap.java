...
public class CogHashmap implements CogPropertyManager {
    ...
    public CogHashmap(HashMap<CogPropertyManager, CogPropertyManager> value) {
        this.value = value;
    }
    public CogHashmap() {
    ...
    public HashMap<CogPropertyManager, CogPropertyManager> getValue() {
        return value;
    }

    private final HashMap<CogPropertyManager, CogPropertyManager> value;
    private static void registerProps(EasyPropManager manager) {
        manager.reg("get", (name, args, script, o) -> {
            CogHashmap hashmap = (CogHashmap) o;
            return hashmap.value.get(args.get(0));
        });
        manager.reg("put", (name, args, script, o) -> {
            CogHashmap hashmap = (CogHashmap) o;
            return hashmap.value.put(args.get(0), args.get(1));
        });
        ...
        manager.reg("containsKey", (name, args, script, o) -> {
            CogHashmap hashmap = (CogHashmap) o;
            return CogBool.getInstance(hashmap.value.containsKey(args.get(0)));
        });
    }
